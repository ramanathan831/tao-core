...
def get_ngc_token(ngc_key: str = "", org: str = "nvidia", team: str = "tao"):
    ...
    response = requests.get(url, headers=headers, auth=auth, params=params)   # noqa pylint: disable=W3101
...
def get_model_metadata_from_ngc(ngc_token: str, org: str, team: str, model_name: str, model_version: str, file: str = ""):
    ...
    response = requests.get(url, headers=headers, params={"page-size": 1000})   # noqa pylint: disable=W3101
    if response.status_code != 200:
...
def get_model_info_from_ngc(ngc_token: str, org: str, team: str):
    ...
    response = requests.get(url, headers=headers, params=params)   # noqa pylint: disable=W3101
    if response.status_code != 200:
