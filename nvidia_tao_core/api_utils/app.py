...
def get_actions(neural_network_name):
    ...
    if not actions:
        metadata = {"error": "Invalid Neural Network Name in get actions", "error_code": 1}
        ...
        200:
          ...
          headers:
            X-RateLimit-Limit:
               $ref: '#/components/headers/X-RateLimit-Limit'
    """
    ep_mappings = module_utils.get_entry_point_module_mapping(neural_network_name)
    if not neural_network_name or neural_network_name not in ep_mappings.keys():
        metadata = {"error": "Invalid Neural Network Name in get schema", "error_code": 1}
        schema = ErrorRspSchema()
        return make_response(jsonify(schema.dump(schema.load(metadata))), 400)
...
def post_action(neural_network_name, action_name):
    ...
    if not module:
        metadata = {"error": "Invalid Neural Network Name in post_action", "error_code": 1}
        schema = ErrorRspSchema()
        return make_response(jsonify(schema.dump(schema.load(metadata))), 400)
    if action_name not in actions:
        metadata = {"error": "Invalid Action Name", "error_code": 2}
    ...
    print({'job_id': job_id, 'data': data, 'neural_network_name': neural_network_name, 'action': action_name, 'status': 'Pending'})
...
def get_job_status(neural_network_name, action_name, job_id):
    ...
    if not module:
        metadata = {"error": "Invalid Neural Network Name in get job status", "error_code": 1}
        schema = ErrorRspSchema()
        return make_response(jsonify(schema.dump(schema.load(metadata))), 400)
    if action_name not in actions:
        metadata = {"error": "Invalid Action Name", "error_code": 2}
