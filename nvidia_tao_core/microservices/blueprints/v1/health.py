...
"""Health check blueprint for API v1 liveness and readiness endpoints."""
...
# v1 Health Blueprint - URL prefix will be set during registration
health_bp_v1 = Blueprint('health_v1', __name__, template_folder='templates')


@health_bp_v1.route('/health', methods=['GET'])
def api_health():
    """api health endpoint"""
    return make_response(jsonify(['liveness', 'readiness']))


@health_bp_v1.route('/health/liveness', methods=['GET'])
@disk_space_check
def liveness():
...
@health_bp_v1.route('/health/readiness', methods=['GET'])
@disk_space_check
def readiness():
