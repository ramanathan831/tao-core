...
class SegFormerHeadConfig:
    ...
    in_channels: List[int] = LIST_FIELD(arrList=[64, 128, 320, 512], description="number of input channels to decoder;be overrided by hard-code in channels in ")  # FANHybrid-S
    ...
    feature_strides: List[int] = LIST_FIELD(arrList=[4, 8, 16, 32], description="Feature strides for the head", display_name="Feature Strides", default_value=[4, 8, 16, 32])  # No change
    ...
    decoder_params: Dict[str, int] = DICT_FIELD(hashMap={"embed_dim": 768}, description="Decoder parameters for the head", display_name="Decoder Parameters", default_value={"embed_dim": 768})  # 256, 512, 768 -> Configurable
...
class SFDatasetSegmentConfig:
    ...
    dataset: str = STR_FIELD(value="SFDataset", default_value="SFDataset", valid_options="SFDataset", description="dataset class")
    ...
    palette: Optional[List[Dict[Any, Any]]] = LIST_FIELD(arrList=[{"label_id": 0, "mapping_class": "foreground", "rgb": [0, 0, 0], "seg_class": "foreground"}, {"label_id": 1, "mapping_class": "background", "rgb": [1, 1, 1], "seg_class": "background"}], description="Palette, be careful of label_transform, if norm then RGB value from 0~1, else 0~255", display_name="Palette")
...
class SFTrainSegmentConfig:
    ...
    weights: List[float] = LIST_FIELD([0.5, 0.5, 0.5, 0.8, 1.0], default_value=[0.5, 0.5, 0.5, 0.8, 1.0], description="Multi-scale Segment loss weight")
