...
class TrainConfig:
    ...
    checkpoint_interval: int = INT_FIELD(value=100, default_value=100, valid_min=1, valid_max="inf", display_name="Checkpoint interval", description="Checkpoint interval. Default is 100.", popular="yes")
    total_checkpoint_limit: int = INT_FIELD(value=0, default_value=0, valid_min=0, valid_max="inf", display_name="Total checkpoint limit", description="Total checkpoint limit. Default is 0.", popular="yes")
    logging_interval: int = INT_FIELD(value=1, default_value=1, valid_min=1, valid_max="inf", display_name="Logging interval", description="Logging interval. Default is 1.", popular="yes")
...
class EvaluateConfig:
    ...
    task: str = STR_FIELD(value="youcook2_val", default_value="youcook2_val", display_name="Task name", description="Task name for evaluation.", required="yes", valid_options="youcook2_val,scienceqa_img")
    model_base: Optional[str] = STR_FIELD(value=None, default_value=None, display_name="Base model name", description="Base model name. This is the original checkpoint if we're evaluating PEFT model.")
    results_dir: Optional[str] = STR_FIELD(value=None, default_value=None, display_name="Results directory", description="Directory for output (defaults to runs/eval/<model_name>/lmms-<task>)")
    num_gpus: Optional[int] = INT_FIELD(value=1, default_value=1, valid_min=1, valid_max="inf", display_name="Number of GPUs", description="Number of GPUS. If not provided, script attempts to detect using `nvidia-smi`.")
...
class InferenceConfig:
    ...
    model_base: Optional[str] = STR_FIELD(value=None, default_value=None, display_name="Base model name", description="Base model name for inference.")
    conv_mode: str = STR_FIELD(value="auto", default_value="auto", display_name="Conversation mode", description="Conversation mode for inference.")
    text: Optional[str] = STR_FIELD(value="What is this video about?", default_value="What is this video about?", display_name="Text input", description="Text prompt for inference.")
    media: Optional[str] = STR_FIELD(value=None, default_value=None, display_name="Media path(s)", description="Path(s) to media files for inference (images, videos).")
