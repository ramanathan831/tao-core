...
class CloudStorage:
    ...
    def __init__(self, cloud_type, bucket_name, region="us-west-1", access_key="", secret_key=""):
        ...
        if cloud_type == "aws":
            apache_driver = get_driver(Provider.S3)
        elif cloud_type == "azure":
            apache_driver = get_driver(Provider.AZURE_BLOBS)
        else:
            raise ValueError("Invalid cloud_type. Supported values: 'aws' or 'azure'.")

        self.driver = apache_driver(access_key, secret_key, region=self.region)
    ...
    def upload_file(self, local_file_path, cloud_file_path):
        ...
        try:
            ...
            print("Uploading", local_file_path, os.path.exists(local_file_path))
            if os.path.exists(local_file_path):
                ...
                if self.is_file(cloud_file_path):
                    ...
                    print("Not Uploaded", local_file_path)
                    raise ValueError(f"File {cloud_file_path} was not uploaded successfully")
        except Exception as e:
            print("Not Uploaded exception", local_file_path)
            raise e
    ...
    def download_file(self, cloud_file_path, local_destination):
        ...
        if not self.is_file(cloud_file_path):
            logger.info("Cloud file {} trying to download doesn't exist".format(cloud_file_path))  # noqa pylint: disable=C0209
            return
        try:
    ...
    def download_folder(self, cloud_folder, local_destination, maintain_src_folder_structure=False):
        ...
        Args:
            ...
            maintain_src_folder_structure: Download the cloud_folder retaining the folder heirarchy present
        """
    ...
    def delete_folder(self, folder):
        ...
        """
        logger.debug("delete folder {}".format(folder))  # noqa pylint: disable=C0209
        try:
    ...
    def delete_file(self, file_path):
        ...
        """
        logger.debug("delete file {}".format(file_path))  # noqa pylint: disable=C0209
        try:
            if self.is_file(file_path):
                obj = self.driver.get_object(container_name=self.bucket_name, object_name=file_path)
                self.driver.delete_object(obj)
            else:
                logger.info("Ffile {} doesn't exist".format(file_path))  # noqa pylint: disable=C0209
        except Exception as e:
            raise e
    ...
    def is_file(self, cloud_path):
        ...
        Args:
            cloud_path: Cloud path to be checked.
        ...
        try:
            ...
            logger.error("File doesn't exist in cloud storage")
            return False
        except Exception as e:
            logger.error("Error checking cloud path: {}".format(e))  # noqa pylint: disable=C0209
            return False
        return True
    ...
    def move_file(self, source_path, destination_path):
        ...
        Args:
            source_path: Source path of the file to be moved.
            destination_path: Destination path for the file.
        """
        logger.debug("moving file {}".format(source_path))  # noqa pylint: disable=C0209
    ...
    def move_folder(self, source_path, destination_path):
        ...
        Args:
            source_path: Source path of the folder to be moved.
            destination_path: Destination path for the folder.
        """
        logger.debug("move_folder {}".format(source_path))  # noqa pylint: disable=C0209
    ...
    def copy_file(self, source_object_name, destination_object_name):
        ...
        Args:
            source_path: Source path of the file to be copied.
        ...
        if self.is_file(source_object_name):
            ...
            try:
                ...
                logger.info("Object copied successfully: {} -> {}".format(source_object_name, destination_object_name))  # noqa pylint: disable=C0209
            except Exception as e:
                logger.error("Error copying object: {}".format(e))  # noqa pylint: disable=C0209
    ...
    def copy_folder(self, source_path, destination_path):
        ...
        Args:
            source_path: Source path of the folder to be copied.
            destination_path: Destination path for the copied folder.
        """
        if not self.is_folder(source_path):
            raise ValueError("Source path must represent a folder.")
    ...
    def create_folder_in_bucket(self, folder):
        ...
        Args:
            folder: Folder path to be created in the cloud storage bucket.
        """
        try:
