...
def add_url_as_release_asset(gitlab_project_id, tag_name, asset_name, asset_url, existing_assets=None):
    """
    Add a URL directly as a release asset.
    
    :param gitlab_project_id: GitLab project ID
    :param tag_name: Release tag name
    :param asset_name: Name for the asset
    :param asset_url: URL to link to
    :param existing_assets: Dictionary of existing asset names to avoid duplicates
    :return: True if successful, False otherwise
    """
    # Check if this asset already exists in the release
    if existing_assets and asset_name in existing_assets:
        print(f"SKIPPED: Asset '{asset_name}' already exists in the release. Skipping duplicate.")
        return True
    
    print(f"Adding URL as release asset: {asset_name} using URL: {asset_url}")
    
    link_data = {
        "name": asset_name,
        "url": asset_url,
        "link_type": "other"  # Use "other" for direct link
    }
    ...
    if curl_with_retry(assets_url, "POST", link_data):
        print(f"Successfully added {asset_name} as release asset link.")
        return True
    else:
        print(f"ERROR: Failed to add {asset_name} as release asset link.")
        return False

def generate_license_acknowledgements(include_apt):
    """
    Generate license acknowledgements for third-party packages.
    
    :param include_apt: Whether to include APT packages in the acknowledgements
    :return: Path to the generated acknowledgements file or None if failed
    """
    print("Generating license acknowledgements...")
    try:
        # Determine command arguments
        cmd = ["python3", "scripts/generate_license_acknowledgements.py", "--output", "acknowledgements.md"]
        if include_apt:
            cmd.append("--include-apt")
            print("Including APT packages in license acknowledgements")
        else:
            print("Not including APT packages in license acknowledgements")
        ...
        if result.returncode == 0:
            print("Successfully generated license acknowledgements")
            output_path = os.path.join(os.getcwd(), "acknowledgements.md")
            if os.path.exists(output_path):
                print(f"Generated file: {output_path}")
                return output_path
            else:
                print(f"ERROR: Generated file not found at expected path: {output_path}")
                return None
        else:
            print(f"ERROR: Failed to generate license acknowledgements. Return code: {result.returncode}")
            print(f"STDOUT: {result.stdout}")
            print(f"STDERR: {result.stderr}")
            return None
    except Exception as e:
        print(f"Exception generating license acknowledgements: {str(e)}")
        traceback.print_exc()
        return None

def commit_license_file(file_path):
    """
    Commit the license acknowledgements file to the repository.
    
    :param file_path: Path to the license acknowledgements file
    :return: True if successful, False otherwise
    """
    print(f"Committing license file {file_path} to repository...")
    try:
        # Configure Git
        subprocess.run(["git", "config", "--global", "user.email", "ci-bot@nvidia.com"], check=True)
        subprocess.run(["git", "config", "--global", "user.name", "TAO CI Bot"], check=True)
        
        # Add the file
        add_result = subprocess.run(["git", "add", file_path], capture_output=True, text=True)
        if add_result.returncode != 0:
            print(f"WARNING: Failed to add {file_path} to Git. Error: {add_result.stderr}")
        
        # Commit the file (don't error if nothing to commit)
        commit_cmd = ["git", "commit", "-m", "Update third-party license acknowledgements [CI SKIP]"]
        ...
        if "nothing to commit" in commit_result.stdout or "nothing to commit" in commit_result.stderr:
            ...
            print(f"WARNING: Failed to commit changes. Error: {commit_result.stderr}")
        else:
            print("Successfully committed changes")
        
        # Push changes (don't error if push fails)
        push_result = subprocess.run(["git", "push", "origin", "HEAD:main"], capture_output=True, text=True)
        if push_result.returncode != 0:
            print(f"WARNING: Failed to push changes. Error: {push_result.stderr}")
        else:
            print("Successfully pushed changes")
        
        return True
    except Exception as e:
        print(f"Exception committing license file: {str(e)}")
        traceback.print_exc()
        return False

def main():
    """Main function to generate and upload license acknowledgements to GitLab."""
    print("Starting license acknowledgements generation and upload process...")
    
    # Parse command-line arguments
    parser = argparse.ArgumentParser(description="Generate and upload license acknowledgements")
    parser.add_argument("--include-apt", action="store_true", help="Include APT packages in the license acknowledgements")
    args = parser.parse_args()
    
    # Print script version for debugging
    print("Script version: 1.0.0 (License Acknowledgements Generator and Uploader)")
    ...
    try:
        ...
        license_file = generate_license_acknowledgements(args.include_apt)
        if not license_file:
            print("ERROR: Failed to generate license acknowledgements. Exiting.")
            return 1
        
        # Create a release in GitLab if it doesn't exist
        if not create_release():
            print("ERROR: Failed to create or find the release. Cannot proceed with asset linking.")
            return 1
        
        # Get existing assets to avoid duplicates
        existing_assets = check_existing_assets(gitlab_project_id, tag_name)
        
        # Add license acknowledgements as a direct URL to the repository
        asset_name = "Third Party Licenses"
        asset_url = f"https://gitlab-master.nvidia.com/-/ide/project/{gitlab_project_id}/blob/{tag_name}/-/acknowledgements.md"
        
        if not add_url_as_release_asset(gitlab_project_id, tag_name, asset_name, asset_url, existing_assets):
            print("WARNING: Failed to add license acknowledgements link to release")
        
        # Commit license file to repository
        commit_license_file(license_file)
        
        print("\nLicense acknowledgements process complete.")
        return 0
